...
void dsp::snd::k_weighting_sos_design(double fs, double sos_num[2][dsp::sos_length], double sos_den[2][dsp::sos_length])
{
	...
	biquad::design(sos_num[0], sos_den[0], biquad::type::high_shelf_eq, DSP_SND_K_WEIGHTING_STAGE1_FC / fs, &s1_g, &s1_q, NULL, NULL);
	...
	biquad::design(sos_num[1], sos_den[1], dsp::biquad::type::highpass, DSP_SND_K_WEIGHTING_STAGE2_FC / fs, NULL, &s2_q, NULL, NULL);
