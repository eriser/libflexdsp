...
using std::fabs; using std::pow; using std::sqrt; using std::cos; using std::sin;
...
struct rpoly_solver
{
	...
	boost::scoped_array<double> p, qp, k, qk, svk;
	...
	int rpoly(const double *op, int degree, double* zeror, double* zeroi, int info[] ) 
	{
		...
		*/
		temp.reset(new double [degree+1]);
		pt.reset(new double [degree+1]);
		p.reset(new double [degree+1]);
		qp.reset(new double [degree+1]);
		k.reset(new double [degree+1]);
		qk.reset(new double [degree+1]);
		svk.reset(new double [degree+1]);
...
namespace {
	...
	struct make_complex {
	std::complex<double> operator()(double re, double im) {return std::complex<double>(re, im);}
};
...
unsigned dsp::rpoly(unsigned degree, const double* poly, std::complex<double>* roots)
{
	...
	zeror.reset(new double[degree]);
	zeroi.reset(new double[degree]);
...
static bool test_rpoly() {
	double poly[] = {4, 3};
	std::complex<double> roots[1];

	unsigned res = dsp::rpoly(1, poly, roots);
	return (1 == res);
}

static const bool test = test_rpoly();
